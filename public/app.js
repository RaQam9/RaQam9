...
const SUPABASE_ANON_KEY = 'eyJhbGciOiJIzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6InV4dHhhdnVyY2dkZXVlZWVtbWRpIiwicm9sZSI6ImFub24iLCJpYXQiOjE3NTEwMjQ4NzYsImV4cCI6MjA2NjYwMDg3Nn0.j7MrIoGzbzjurKyWGN0GgpMBIzl5exOsZrYlKCSmNbk';
...
function initializeAuth() {
    ...
    const showView = (view) => {
        [loginView, signupView, loggedinView].forEach(v => v.style.display = 'none');
...
async function initializePredictionsPage() {
    ...
    try {
        ...
        if (data) {
            ...
            container.innerHTML = '<p class="text-center text-gray-400 mt-8">أنت غير متصل. سيتم عرض البيانات من النسخة المحفوظة.</p>';
        }
    } catch (error) {
        console.error("Error fetching predictions:", error);
        container.innerHTML = '<p class="text-center text-red-500 mt-8">فشل تحميل المباريات. اسحب للأسفل للتحديث.</p>';
    }
}
...
function initializeAppWithData(matchesData) {
    ...
    const upcomingMatches = matchesData.filter(m => new Date(new Date(m.datetime).toLocaleDateString('en-CA')) >= today);
    const matchesByDay = upcomingMatches.reduce((acc, m) => {
        const day = new Date(m.datetime).toLocaleDateString('en-CA');
        if (!acc[day]) acc[day] = [];
        acc[day].push(m);
        return acc;
    }, {});
    ...
    try {
        if (form.name === 'prediction-form') {
            ...
            const { error } = await supabaseClient.from('predictions').upsert({
                ...
                predicted_scorer: form.scorer.value.trim()
...
async function loadUserPredictions() {
    ...
    data.forEach(p => {
        ...
        if (form) {
            ...
            form.scorer.value = p.predicted_scorer || '';
            [...form.elements].forEach(el => el.disabled = true);
...
async function initializeNewsPage() {
    ...
    try {
        ...
        if (data) {
            ...
            articlesGrid.innerHTML = '<p class="text-center text-gray-400 col-span-full">أنت غير متصل. سيتم عرض الأخبار من النسخة المحفوظة.</p>';
        }
    } catch (error) {
...
function refreshVisibleComments() {
    ...
    if (!articlePage.style.transform || !articlePage.style.transform.includes('100')) {
...
function renderComments(container, commentsData, tableName) {
    ...
    // Logic for nesting comments can be added here if needed
    commentsData.forEach(comment => {
        const commentEl = document.createElement(tableName === 'comments' ? 'div' : 'div');
        ...
        if (tableName === 'comments') { // Match comments layout
            commentEl.innerHTML = `
...
async function fetchAndRenderProfileComments() {
    ...
    listDiv.innerHTML = allComments.map(c => {
        ...
        return `<div class="profile-comment-item" id="profile-comment-${c.id}-${c.table}">
...
function initializeBackButtonHandler() {
    ...
    App.addListener('backButton', () => {
        ...
        if (profilePage.classList.contains('is-visible')) { closeProfilePage(); return; }
        if (authModal.classList.contains('show')) { authModal.classList.remove('show'); return; }
